...
export default function FooterLayer1Page() {
  const [links, setLinks] = useState<Link[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    fetch('/api/footer/layer-1')
      .then(r => r.json())
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      await fetch('/api/footer/layer-1', {
        method: 'POST',
    ...
    <div className="p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-semibold text-text-primary">Footer Katman 1 - Linkler</h1>
            <p className="text-text-secondary mt-1">Üst footer linklerini yönetin</p>
          </div>
